...
import { TrendingUp, Info, ChevronDown, ChevronUp, Zap } from 'lucide-react'
      ...
      <div className="bg-gradient-to-br from-emerald-500/10 to-cyan-500/10 border border-emerald-400/30 rounded-2xl p-8 relative overflow-hidden text-center shadow-lg shadow-emerald-900/10">
        ...
        <div className="relative z-10">
          ...
          <p className="mt-4 text-sm text-emerald-300">
            Single-model estimate generated with {analysis.details.primary_label}.
          </p>
        </div>
      </div>
        ...
        {showDetails && (
          <div className="mt-4 bg-black/20 rounded-xl p-6 border border-white/5 animate-in fade-in slide-in-from-top-2">
            <div className="flex gap-2 mb-4">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium bg-emerald-600/20 text-emerald-300 border border-emerald-500/30">
                <Zap className="w-3 h-3" />
                {analysis.details.primary_label}
              </div>
            </div>

            <div className="prose prose-sm prose-invert max-w-none text-gray-400">
              <ReactMarkdown>{analysis.details.primary_summary || 'No reasoning details available.'}</ReactMarkdown>
            </div>
          </div>
        )}
      </div>
